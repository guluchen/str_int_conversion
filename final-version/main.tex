...
to explain the main ideas behind our decision procedure.  To make our terminology explicit: $\Phi$ states that \changed{$``0''$ concatenated with $x$ is the same as $x$ with $``0''$, the numeric value of the string $x$ is equivalent to that of $y$, $y$ is longer than $x$, and $y$ is longer than $1000$.  Notice that $\Phi$ is satisfiable. E.g., it has a model $x=``00"$ and $y=``0^{1002}"$}.
