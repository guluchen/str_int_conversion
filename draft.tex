...
\begin{abstract}
String-number conversion is an important class of constraint needed for the symbolic execution for programs. It is particular important in the analysis of scripting languages such as JavaScript and Python, where string-number conversion is a part of the definition of the core semantics of the language. However, the state-of-the-art constraint solvers have only very limited support to this type of constraint. In this paper, we propose an approach that can support a combination of string-number conversion as well as other common types of string constraints efficiently. Experimental results show that it significantly outperforms other state-of-the-art tools on benchmarks that involves string-number conversion.
\end{abstract}
 \setcopyright{none} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
...
every parametric word $w\in L(A)$ is uniquely determined by its Parikh image $\parikhof{w}$ due to the flatness. 
...
\emph{Parametric flat automaton (PFA)} is then a parametric automaton whose  automaton is flat. See Figure~\ref{fig:sfa_def} and~\ref{fig:sfa} for examples of PFAs (only the automata component is shown
).
...
%
The flattening $\outlf$ will be built inductively following the structure of $\insc$.
...
At the end we set $\outlf \defeq \underf \dr \insc$.
...
We will now discuss the special form of $\dr(x)$, called numeric PFA, which we choose for string variables appearing in string-integer constraints and that leads to efficient underapproximation technique.
%
Besides simple induced linear formulae, we still want single $\dr(x)$ to cover as many numerals as possible.
We want an ``easy" completeness property, that is,  the space of all numerals can be covered completely by our special numeric PFAs, and that these numeric PFAs are generated easily, and each of them covers a large and practically significant portion of numerals (so that satisfiable assignments can be often found within the domain restriction of just few of numeric PFAs).
%
...
%
A last problem still needed to be solved beofe they can be efficient in practice. 
%
