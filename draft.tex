...
a \emph{flat domain restriction} for $\insc$ is a mapping $\dr$ that assigns to every variable $x\in X$ a PFA $\dr(x)$ over character variables $V_x$ \changed{ and we let $V_\dr = \bigcup_{x\in X} V_x$ be the set of all character variables used in $\dr$.}
...
\yfc{I do not understand the previous sentence}
\lh{it is better?}
...
The set of all $\dr$-encodings of $(I\cup I')$ is then denoted $\encode \dr {(I\cup I')}$
...
Given a length constraint $\phi_l$ that talks about lengths $|x|$ of string variables $x\in X$. 
We use a version of $\phi_l$ where every occurence of every leght term $|x|$ is replaced by an auxiliary lenght variable $\lx$ and add a formula to ensure that the value of $\lx$ is equal to that of $|x|$ even when $x$ is encoded using the character variables $V_x$ and Parikh variables $\# V_x$ of $\dr(x)$.
%
