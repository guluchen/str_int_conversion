...
However, this constraint contains an exponential component $100^{\#(v_0)}$. To the best of our knowledge, the satisfiability problem of integer constraints with a mix of polynomials and exponentials is still open. For a more general form, if we restrict the domain of $x$ to the SFA in Figure~\ref{fig:sfa_def}, for the case when $n$ is positive, we have the following relation.

\[\begin{array}{lcl}
y & = & (v_m^0\times 10^3 + v_m^1 \times 10^2 + v_m^2 \times 10 +v_m^3 )\times\\
&   & \frac{10000^{\#(v_m^0)+1}-1}{10000-1}+\\
&  & v_{m-1}\times 10000^{\#(v_m^0)+1}\\

&   &+\ldots+ \\

&   &v_1\times10000^{\#(v_m^0)} \times \ldots \times 10000^{\#(v_1^0)}+\\
&   &(v_0^0\times 10^3 + v_0^1 \times 10^2 + v_0^2 \times 10 +v_0^3 )\times\\
&   &\frac{10000^{\#(v_0^0)}-1}{10000-1} \times 10000^{\#(v_m^0)} \times \ldots \times  10000^{\#(v_1^0)} \times 10
\end{array}\]


Recall that $\#(x_1)$ is the number of occurrence of $v_1$. To solve the satisfiability of this formula, one need to solve an exponential constraint. 
