...
%For many languages such as Java, string-number conversion is not a core operation, and much symbolic execution can be done without supporting it; however, this is not true of scripting languages, of which JavaScript is the most notable.
%For many languages such as JavaScript, string-number conversions are core operations, and symbolic execution cannot be done without supporting it.
In fact, a code that receives string input tends to need to convert at least some of that input into numbers.
%,  which makes the handling of string-integer conversions of the utmost importance.  \yfc{maybe this is a too strong argument using this example}
...
We say that a mapping $I_e:\cvar\cup\#\cvar \rightarrow \nat$ is a \emph{word encoding} a word $w$  (or a $\pa$-encoding of $w$) if $w$ is an instantiation of some parametric word $x \in L(\pa)$ whose Parikh image and interpretation of character variables are defined by $I_e$.  
%
...
% to \emph{word encodings} $e:\cvar\cup\#\cvar \rightarrow \nat$ via the PA $\pa$ and the function $\decode \pa$ does it in the reverse direction.
	$$\encode \pa( w ) = \{I_e \mid  x \in L(\pa) , I(x) =w, I\in\semof \paf, I_e = I \cup \parikhof{x}\}$$
	$$\decode \pa(I_e) =\{ w \mid x \in L(\pa) , I(x) =w, I\in\semof \paf, I_e = I \cup \parikhof{x} \}$$
...
If $\pa$ is a PFA, then by Lemma~\ref{lemma:decoding}, each word encoding $I_e\in \encode \pa( w )$ can be decoded uniquely to the word $w$, i.e.
$$
...
\hide{Plan to remove this one and use the orange one.
...
The function takes as an argument an $\dr$-encoding of a string constraint, an interpretation $I_e \cup I_z$ consisting of a \emph{many-word encoding} $I_e: V_\dr\cup\#V_\dr\rightarrow \nat$ and an integer variable interpretation $I_z$. 
Intuitively, the many-word encoding $I_e$ contains for each $x\in X$ a $\dr(x)$-encoding of its value. 
The function then returns the set of string constraint interpretations 
$\decode  \dr (I_e\cup I_z) \defeq \{I_s \cup I_z \mid \forall x\in X: I_s(x) = \decode {\dr(x)} ((I_e)_{V_{\dr(x)}\cup \#V_{\dr(x)}})\}$.
The condition says that each string variable $x$ is interpreted as the ${\dr(x)}$-decoding of the word encoding $(I_e)_{V_{\dr(x)}\cup \#V_{\dr(x)}}$.
...
%
namely, for a model $I_s \cup I_z$ of a string constraint,  
we let $\encode \dr (I_s \cup I_z) =\{I_e\cup I_z\mid \decode \dr (I_e\cup I_z) = I_s \cup I_z \}$.
...
The correctness of the entire construction of $\underf \dr {\insc}$ is expressed by  Theorem~\ref{theorem:correct}. 
It uses the decoding function $\decode \dr$ parameterized by the domain restriction $\dr$. 
The $\decode \dr$ function maps a \emph{ many-words encoding} $I_e: V_\dr\cup\#V_\dr\rightarrow \nat$ paired with an interpretation of integer variables $I_z$ to a model $I_s \cup I_z$ of a string constraint. Note that the many-words encoding $I_e$ is a union of ${\dr(x)}$-encodings for all $x$ in $X$.
Formally, 
$\decode  \dr (I_e\cup I_z) = \{I_s \cup I_z \mid \forall x\in X: I_s(x) = \decode {\dr(x)} ((I_e)_{V_{\dr(x)}\cup \#V_{\dr(x)}})\}$.
The condition says each string variable $x$ is interpreted as the ${\dr(x)}$-decoding of the word encoding $(I_e)_{V_{\dr(x)}\cup \#V_{\dr(x)}}$.
%We define the output of $\decode  \dr (I_e\cup I_z)$ as a set so that \emph{invalid word encodings}, i.e. those do not corresponds to possible word encodings from PFAs, are mapped to empty sets and valid ones are mapped to singleton sets.
We define the $\dr$-encoding function as the counterpart of $\dr$-decoding,  
%
namely, for a model $I_s \cup I_z$ of a string constraint,  
we let $\encode \dr (I_s \cup I_z) =\{I_e\cup I_z\mid \decode \dr (I_e\cup I_z) = I_s \cup I_z \}$.
We lift $\decode \dr$ and $\encode \dr$ to sets of interpretations in the standard manner.
...
\{I_e \cup I_e'\mid I_e \in \encode \pa (\semof{\pa}) \wedge I_e' \in\encode {\pa'} (\semof{\pa'})\}$.
\end{lemma}
