...
$$\Psi_n\defeq (n=-1 \vee \bigvee_{k\in [0,m]} (\mathsf{epsAfter(k)} \wedge \mathsf{toNumber(k)} ) )$$
...
\begin{lemma}\label{lemma:sticorrect_neg}
	$\semof {\Psi_n \wedge \parikhfof {A_x} \wedge (v_0 = 0)\wedge (n = -1) }_{V_x \cup \#V_x \cup \{n\}} = \{I_n\mid (I_n(n) = -1\} \times \semof {\parikhfof {A_x} \wedge (v_0 = 0) }_{V_x \cup \#V_x} $
\end{lemma}
In this case, $x$
