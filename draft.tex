...
Namely, an PFA is a finite automaton $A = (Q,T,\cvar,\st{0}{0},\st{m}{0})$ operating over the alphabet $\cvar$ of \emph{character variables}, 
...
We may say that $\parikhof x$ is an encoding of $x$, and define the decoding function $\decode$ such that $\decode(\parikhof x) = x$.
It is well known that $\parikhof{A}$ can be computed in the form of a linear arithmetic formula  $\pformulaof{A}$ such that $\modelsof{\pformulaof A} = \parikhof{A}$, in time linear to the size of $A$ \cite{}.
\begin{lemma}\label{lemma:decx}
For a PFA $A$, $L(A) = \{\decode(\pim) \mid \pim\in\semof{\pformulaof A}\}$.
\end{lemma}


Since every string $w \in \semof{A}$ is obtained from a parametric string $x\in L(A)$ and an interpretation of character varriables $I:\cvar\rightarrow\Sigma_\epsilon$ such that $w = I(x)$, and since $x = \decode(\parikhof x)$, 
we may say that the assignment $I \cup \parikhof x$ uniting the interpretation $I$ and the Parikh image of the parametric string $x$ encode $w$. Lemma~\ref{lemma:parikh} also implies that two different strings are encoded by different united assignments (the Parikh image or the interpretation must be different), and hence we may extend the decoding function $\decode$ to united assignments so that $\decode(I \cup x) = w$.
Encodings of strings in $\semof{A}$ are encodings of strings of $L(A)$ paired with arbitrary interpretations of $\cvar$, 
hence $\semof A$ is characterised by models of $\parikhof A$ interpreted over variables $\cvar\cup\pvars$ (variables of $\cvar$  may have arbitrary values since $\parikhof A$ does not constraint them): 
%\begin{lemma}\ref{lemma:decw}
%For a PFA $A$, $\semof{A} = \{\decode(I\cup \pim) \mid \pim\in\semof{\pformulaof A} \land I:\cvar\rightarrow \Sigma_\epsilon\}$.
%\end{lemma}
\begin{lemma}
For a PFA $A$, $\semof{A} = \{\decode(I\cup \pim) \mid I\cup\pim\in\semof{\pformulaof A}_{\cvar\cup\pvars}\}$.
\end{lemma}

\paragraph{Synchronisation of PFA}
The following construction of the so called synchronisation formula of two PFA $A$ and $B$ is a crucial component of bulding overapproximations of string constraints. 
Synchronisation formula is a formula $\syncfof A B$ over $\pvars\cup\cvarone\cup \cvartwo$ encoding the semantic intersection $\semof A \cap \semof B$. Besides the intersection, it contains a more precise information about the synchronisation of $A$ and $B$, 
namely, it characterises the set of pairs of encodings $(I\cup \pim)\in \parikhof A$ and $(I'\cup \pim')\in\parikhof B$ such that 
$\decode(I\cup \pim) = \decode(I'\cup \pim')$ which constitute the intersection.

Assume that $A = (Q,T,\cvarone,q_i,q_f)$ and $B = (P,T',\cvartwo,p_i,p_f)$ are two SFA over disjoint sets of variables, $\cvarone \cap \cvartwo = \emptyset$. 
We build so called asynchronous product automaton $\syncof A B$ of $A$ and $B$ and then extract the formula $\syncfof A B$ from its Parikh image.
%
The product automaton uses $Q\times P$ as the set of states and $(\cvarone \cup \{\epsilon\}) \times (\cvartwo \cup \{\epsilon\} )$ as the alphabet. If a transition $((q_1,p_1), (v,v'),(q_2,p_2))$ is taken, it means the character variable $v$ and $v'$ should be assigned the same value and this value leads $A$ from state $q_1$ to state $q_2$ and $B$ from $p_1$ to $p_2$.
...
\begin{itemize}
\item If $(q_1,v,q_2) \in T$, then for all states $p\in P$, we have $((q_1,p),(v,\epsilon),(q_2,p))\in \syncT$.
\item If $(p_1,v,p_2) \in T'$, then for all states $q\in Q,((q,p_1)$, we have $(\epsilon,v),(q,p_2))\in \syncT$.
\item If $(q_1,v,q_2) \in T$ and $(p_1,v',p_2) \in T'$, then we have $((q_1,p_1),(v,v'),(q_2,p_2))\in \syncT$.
\end{itemize}	
The first two cases correspond to the situation when the character variable $v$ is assigned $\epsilon$. In such cases, only one automaton changes its state.
%
We compute a Presburger formula $\pformulaof {\syncof A B}$ over the set of variables $\{\#(v,v')\mid (v,v') \in \cvarone \times \cvartwo\}$ of the product automaton. 
%We use $\#v$ and $\#v'$ to denote the number of occurrences of $v_i$ and $v_i^{(j)}$, respectively. 
The following formulae establish the relation of the number of occurrences of symbols between the SFAs and the product automaton.
\begin{enumerate}
	\item for all $v \in \cvarone$, $\#(v) = \sum_{v' \in {\cvartwo \cup \{\epsilon\}}} \#{(v,v')}$   
	\item for all $v \in \cvartwo$, $\#(v) = \sum_{v' \in {\cvarone \cup \{\epsilon\}}} \#{(v',v)}$ 
\end{enumerate}
...
\begin{enumerate}
	...
	Mitem for all $v\in \cvartwo$,$\#{(\epsilon,v)}>0 \rightarrow (v=\epsilon)$.
\end{enumerate}

The correctness of this construction is expressed by the following lemma
\begin{lemma}
For $\pim:\pvars\rightarrow \nat$, $\pim':\pvars\rightarrow\nat$, $I:\cvarone\rightarrow\Sigma$, $I':\cvartwo\rightarrow\Sigma$, 
we have $(I\cup\pim)\cup(I'\cup\pim')\in\semof \syncofof A B$ if and only if $\decode(I\cup\pim) = \decode(I'\cup\pim')$.
\end{lemma}
We actually have even something stronger, namely, that, $\syncofof A B$ has all possible allignments. How do I say this ... or do I have to say this?
...
§$(\cvarone \cup \{\epsilon\}) \times (\cvartwo \cup \{\epsilon\} )$ as the alphabet. If a transition $((q_1,p_1), (v_i^{(j)},v_{i'}^{(j')}),(q_2,p_2))$ is taken, for some $i,j,i',j' \in \mathbb{N}$, it means the character variable $v_i$ and $v_{i'}$ (and so all of their indexed variants) should be assigned the same value and this value leads $A$ from state $q_1$ to state $q_2$ and $B$ from $p_1$ to $p_2$.
