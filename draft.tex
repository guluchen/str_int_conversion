...
\begin{abstract}
String-number conversion is an important class of constraint needed for the symbolic execution of string-manipulating programs. In particular solving string constraints with string-number conversion  is necessary  for the analysis of scripting languages such as JavaScript and Python, where string-number conversion is a part of the definition of the core semantics of these languages. However, the state-of-the-art constraint solvers have only very limited support to this type of constraint. To overcome these limitations, we propose, in this paper, an approach that can support a combination of string-number conversion as well as other common types of string constraints efficiently. Experimental results show that it significantly outperforms other state-of-the-art tools on benchmarks that involves string-number conversion.
\end{abstract}
 \setcopyright{none} 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
\begin{document}
...
%
%However,  solvers have limited support for operations that convert between strings and integers; string length operations are sometimes well supported, but not parsing a string as an integer or turning an integer value into its string form. When there is support, it is often very limited. Thus, $n = \sti{\texttt{"2"}}$ or $x = \its{5}$ are handled but $n = \sti{x}$ (i.e., converting the string $x$ to the integer $n$) and $x = \its{n}$ (i.e., converting the integer $n$ to the string $x$) are core operations %
%
...
%Currently, even for the above constraint, which is from a simple program, most of the-state-of-the art solvers fail to provide a correct answer. 
...
%Solving string constraints with string-number conversion has been proven undecidable~\cite{day2018satisfiability}, and our experiments (Section \ref{section:evaluation}) show that state-of-the-art string constraint solvers provide only limited support to string-number conversion. 
...
%Observe that our main contribution lies in the 
...
%Such reduction based on PFA is the key to handling string-number conversion efficiently. When we restrict the solution space to those defined by PFAs, the original undecidable problem (i.e., string constraint with string-number conversion) is reduced to the satisfiability problem of linear formulae and can be solved efficiently. 
...
%Such reduction based on PFA is the key to handle string-number conversion efficiently. When we restrict the solution space to those defined by PFAs, the original undecidable problem (i.e., string constraint with string-number conversion) is reduced to linear formulae and can be solved efficiently. We can step-wisely enrich the PFAs and hence solve the satisfiability problem of string constraints in a systematic manner.
...
%For over-approximation, there are a few existing algorithms~\cite{abdulla2019chain,z3,chen2019decision}, our framework does not set any restriction on which algorithm to be used. In our experiences, the UNSAT instances in all available benchmarks are not that difficult, and the differences between the major algorithms are very small. In our tool \tool, we use a version that is a simple adaption of the algorithm in~\cite{abdulla2019chain}.
...
%More specifically, if we want to say $n \in \varn$ is the integer value of the string $x \in \Sigma^*$, we can write the following constraint (ignore the case that $x$ is ``not a number", which can also be encoded using more conditions):
...
%
%In order to explain this step, we need to first describe what is a \emph{chain} in a set of equality constraints.
%For two equality constraints, 
%$\begin{array}{rcll}
%	t_1&\defeq& \ldots x\ldots = \ldots y\ldots\\
%	t_2&\defeq& \ldots y \ldots = \ldots&,
%\end{array}$\\
%we say the $x$ in $t_1$ \emph{depends on} the $y$ in $t_2$, because $y$ occurs on the other side of $x$ in $t_1$ and occurs on the same side of $x$ in $t_2$. 
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
...
This is possible thanks to the flatness, which causes that 
...
First observe that the $\parikhof w$ value of any variable appearing within the a cycle of $A$ is equal to the number of repetitions of that cycle in the accepting run. This is an immediate consequence of the fact that 
 every variable appears on at most one transition. Thus, the accepting run on $w$ (and so $w$ itself) can be reconstructed from $\parikhof w$.  More concretely, the function $\decode A$ can be implemented as follows. 
...
%
%We define the semantics of $x$ as the set $\semof x$ of all possible interpretations of $x$, 
%the semantics of a language $L\subseteq V^*$ is the union $\semof L = \cup_{x\in L} \semof{x}$ of semantics of its words, 
...
\hide{Some attempts to define both encodes and decodes of PA, need more explanation of why we define them I guess. It is planed to be a replacement of the text below him. One interesting observation: the definitions of both are almost the same
	$$\encode \pa( w ) = \{e \mid  x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x}\}$$
	$$\decode \pa(e) =\{ w \mid x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x} \}$$
	}
\changed{
We say that a mapping $e:\cvar\cup\#\cvar \rightarrow \nat$ is a \emph{word encoding} a word $w$ (or a $\pa$-encoding of $w$) if $e = \parikhof{x}\cup I$ for
for some $x \in L(A)$ and $I\in\semof\paf$ such that $w = I(x)$.   
%
Conversely, $w$ is a \emph{$\pa$-decoding} of $e$.
%
We use $\encode \pa(w)$ below to denote all $\pa$-encoding of a word $w$,
and $\decode \pa (e)$ to denote all $\pa$-decodings of a word encoding $e$. Namely,  
% to \emph{word encodings} $e:\cvar\cup\#\cvar \rightarrow \nat$ via the PA $\pa$ and the function $\decode \pa$ does it in the reverse direction.
	$$\encode \pa( w ) = \{e \mid  x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x}\}$$
	$$\decode \pa(e) =\{ w \mid x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x} \}$$
Informally, that a word encoding $e$ belongs to $\encode \pa( w )$ means that $w$ is an interpretation of a parametric word $x \in L(\pa)$ whose Parikh image and interpretation of character variables are defined by $e$. 
Since $e$ only records the numbers of occurrences of character variables (Parikh image), 
the same word encoding may be shared by multiple words, as formalized in the definition of $\decode \pa(e)$. 

If $\pa$ is a PFA, then by Lemma~\ref{lemma:decoding}, each word encoding $e\in \encode \pa( w )$ can be decoded uniquely to the word $w$, i.e.
$$
w = \decode \pa (\encode \pa (w))
$$
Consequently, as stated by the following corollary of Lemma~\ref{lemma:decoding}, 
...
%words in $\semof{\pa}$ are precisely the decodings of the Parikh image of parametric words in $L(A)$ paired with character variables interpretations satisfying $\paf$:
}	

%The function $\encode \pa$ below encodes a word $w$ to \emph{word encodings} $e:\cvar\cup\#\cvar \rightarrow \nat$ via the PA $\pa$ and the function $\decode \pa$ does it in the reverse direction.
%	$$\encode \pa( w ) = \{e \mid  x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x}\}$$
%	$$\decode \pa(e) =\{ w \mid x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x} \}$$
%Informally, a word encoding $e\in \encode \pa( w )$ means $w$ corresponds to a parametric word $x \in L(\pa)$ whose Parikh image and interpretation of character variables are defined by $e$. Since $e$ has only the information of number of occurrences of character variables (Parikh image), the same word encoding may be shared by multiple words, as formalized in the definition of $\decode \pa(e)$. For the case that $\pa$ is a PFA, by Lemma~\ref{lemma:decoding} each word encoding $e\in \encode \pa( w )$ uniquely encodes the word $w$. The following corollary of Lemma~\ref{lemma:decoding} states that words in $\semof{\pa}$ are decodings of the Parikh image of parametric words in $L(A)$ paired with character variables interpretations satisfying $\paf$:

\hide{
	Observe that every word $w \in \semof{\pa}$ is uniquely determined by a parametric word $x\in L(A)$ and an interpretation $I\in\semof{\paf}$ such that $w = I(x)$. We call the union of $I$ and the Parikh image $\parikhof{x}$ a \emph{word encoding} of $w$ in $\pa$. More formally, a wording encoding of $\pa$ is mapping of the type $\cvar\cup\#\cvar \rightarrow \nat$ and the set of word encodings of $w$ in $\pa$ is defined as $\encode \pa( w ) = \{e \mid  x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x}\}$. For every word encoding $e\in \encode \pa( w )$, there must be a parametric word $x\in L(P)$ and some valid character assignment $I\in \semof \paf$ such that $I(x) =w$ and $e= I \cup \parikhof{x}$. When $e\in \encode \pa( w )$, we also say it \emph{$\pa$-encodes} $w$. We lift the encoding function $\encode \pa$ to set of word $W$ by defining  $\encode \pa(W) = \bigcup_{w\in W} \encode \pa(w)$.
	
	Conversely, given a word encoding $e:\cvar\cup\#\cvar \rightarrow \nat$, the decoding of $e$ in $\pa$ is defined as the set $\decode \pa(e) =\{ w \mid x \in L(\pa) , I(x) =w, I\in\semof \paf, e = I \cup \parikhof{x} \}$. We lift the decoding function $\decode \pa$ to set of word encodings $E$ by defining  $\encode \pa(E) = \bigcup_{e\in E} \decode \pa(e)$.
	

	When $\pa$ is flat, Lemma~\ref{lemma:decoding} implies that such $\pa$-encoding can be uniquely decoded. The following corollary of Lemma~\ref{lemma:decoding} states that words in $\semof{\pa}$ are decodings of the Parikh image of parametric words in $L(A)$ paired with character variables interpretations satisfying $\paf$:
	

}
\yfc{Plan to remove this one and use the orange one.
	
Observe that every word $w \in \semof{\pa}$ is uniquely determined by a parametric word $x\in L(A)$ and an interpretation $I\in\semof{\paf}$ such that $w = I(x)$. 
