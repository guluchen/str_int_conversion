...
Operations such as \emph{equality constraints} (e.g. $x.y = y.x$), \emph{regular constraints} (e.g., $x \in (a.b)^*$), and \emph{integer constraints} (e.g., $|x|-|y|>3$), are widely supported by most state-of-the-art string constraint solvers such as, CVC4 \cite{?}, OSTRICH \cite{}, Sloth \cite{},  Trau+ \cite{abdulla2018trau,parosh2019chain,abdulla2017flatten}, Z3 \cite{?} and Z3Str3 \cite{}. 

An important class of string operations is  the string-number conversions.  While string length operations are sometimes well supported, converting a string $x$ to an integer $n$ (e.g., using the operation $n = \sti{x}$) or turning an integer value $n$ into its string form $x$ (e.g., using the operation $x = \its{n}$)  suffer from extremely limited support by the state-of-the-art string constraint solvers. In fact, for many languages such as Java, string-number conversions are core operations, and much symbolic execution can not been done without supporting it. In many security vulnerable codes,  that receive input, tend to need to convert at least some of that input into numbers, which makes string-integer conversions critical to checking it. \yfc{some grammar error?} For example, the program fragment below is a variant of the Luhn test algorithm \cite{?} that is often used in credit card or personal ID validation.
...
To check whether the program path that traverses both loop exactly once, which corresponds to the case when $| \verb|value||=2$, and finally passes this test has a valid input, we create the following (string) constraint:
...
Even, more crucially, in many programming languages, the conversion between strings and integers is a part of the definition of the core semantics of the language. 
...
A casual glance at the above code reveals no use of strings at all, but the semantics of field access is somewhat unusual in JavaScript: the arrays are indexed by strings, and numeric indices are converted to strings. This conversion is mandated explicitly by the JavaScript semantics: the 2019 edition of ECMAScript \cite{?} requires that $ToPropertyKey$ be called on the element expression (\S{12.3.2.1}), and $ToPropertyKey$ calls {\tt{ToString}} on that value in all but special cases (\S{7.1.14}). Any faithful symbolic execution of JavaScript must handle such conversions for even basic array operations to work correctly. Consider the following code snippet that manipulates an array \texttt{x}, with its value shown on the right:
...
Observe that if the over-approximation is UNSAT then our decision procedure  declares that the original formula is also UNSAT and terminates. Surprisingly, despite its simplicity, our over-approximation procedure works very well in practice. Coming back to the formula $\Phi$, the over-approximation module will  return SAT  in this case. 
...
\item The class of \emph{parametric flat automata} which is the key for efficient handling of string-number conversion constraints.
...
\scalebox{0.7}{
	...
	\node   (m2) [above right = -0.4cm and -0.3cm of q13] {$\ ^{-1}$};
	
	\node[state] (qm1) [above left = 0.6cm of qm] {$\st{m}{1}$};
	\node[state] (qm2) [above = 1cm of qm] {$\st{m}{2}$};
	\node[state] (qm3) [above right = 0.6cm of qm] {$\st{m}{l_m}$};
	\node   (mm) [above right = -0.4cm and -0.3cm of qm3] {$\ ^{-1}$};
...
\lh{, allowing to circumvent more costly automata algorithms and decision procedures. Concertize and justify this better?}\yfc{maybe we can say according to the experience of verification of concurrent system, flattening is usually faster than pure automata operation? Faouzi should know something.}
%
...
so that $\decode \pa(I \cup \parikhof x) = w$ (recall that $\Sigma_\epsilon \subseteq \nat$). \yfc{true only if we move the semantic of $\epsilon$ to preliminary. Now it is a symbol.}
...
a \emph{flat domain restriction} for $\insc$ is a mapping $\dr$ that assigns to every variable $x\in X$ a PFA $\dr(x)$ over character variables $V_x$.
...
$\decode R (\semof {\outlf}) = \semof{\insc}^\dr$ 
\end{theorem}

The theorem can be proved by a structural induction over $\insc$.
However, for the induction step to go through, we will need a more meticulous view into the correspondence of $\phi$ and $\underf \dr \phi$ than just equality of their semantics. \yfc{I do not understand the previous sentence}
...
To formalize this correctness argument, we define an additional notation.
For an interpretation $I$ of string variables, 
we define its $\dr$-encoding as the set $\encode \dr(I) = \bigcup_{x\in X} \encode {\dr(x)} (I(x))$ of interpretations of $V_\dr\cup \#V_\dr$, containing combinations of interpretations of character variables of each $\dr(x)$ that encode the $I$. \yfc{the sentence is difficult to parse. I did not find the definition of $V_\dr\cup \#V_\dr$.}
%
\changed{
Further, we define an $\dr$-encoding for an interpretation $I \cup I'$ of a general string constraint, consisting of an interpretation $I$ of string variables and an interpretation $I'$ of integer variables.
%
An $\dr$-encoding of $I \cup I'$ is a union  $e \cup I'$ of an encoding $e\in \encode \dr(I)$ and the original integer interpretation $I'$ which satisfies that for every string length variable $|x|$ in the domain of $I'$,
$|x| = \sum_{v\in V_x,I(v)\neq\epsilon}\#v$. 
That is, the interpretation of string variables is encoded as usual, the interpretation of integer variables is kept as is, 
and for each string length variable $|x|$ in the domain of $I'$, 
the value of $|x|$ is consistent with the encoding of $x$ by variables $V_x$ and $\#V_x$.
The set of all $\dr$-encodings of $(I\cup I')$ is then denoted $\encode \dr {(I\cup I')}$}\yfc{need a serious rewrite :-) Maybe I need to know why you define so many encodings...}
%
