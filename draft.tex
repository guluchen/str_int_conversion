...
Symbolic execution is a very popular technique that allows the programmers to check the feasibility of a path in a given program (i.e., determining the value of the inputs under which the given path can be executed) \cite{????}).
The path feasibility problem is usually solved by a reduction to the satisfiability problem of a formula. More precisely, program statements in the path are usually translated to equivalent constraints in static single assignment (SSA) form and then solved by \emph{satisfiability module theory (SMT)}  solvers. The types of constraints that needed to be solved by the SMT solvers depend on the type of program variables. Therefore, there is a need for SMT solvers that support different combinations of theories (i.e., depending on the types of the manipulated variables). 

Among all types of program variables, \emph{string data type}  is omnipresent in modern programming languages.  Various infamous security vulnerabilities such as injection and cross-site scripting attack are caused by malicious string values.  Therefore, string constraint solving has received considerable attention in the constraint solving community.
Operations such as \emph{equality constraints} (e.g. $x.y = y.x$), \emph{regular constraints} (e.g., $x \in (a.b)^*$), and \emph{integer constraints} (e.g., $|x|-|y|>3$), are widely supported by most state-of-the-art string constraint solvers such as Z3  \cite{?}, CVC4  \cite{?}, Z3Str3  \cite{?}, and Trau+ \cite{?}. 
 However,   solvers have limited support for operations that convert between strings and integers; string length operations are sometimes well supported, but not parsing a string as an integer or turning an integer value into its string form.  When there is support, it is often very limited. Thus, $n = \sti{\\texttt{"2"}}$  or $x = \its{5}$  are handled but $n = \sti{x}$ (i.e., converting the string $x$ to the integer $n$) and  $x = \its{n}$ (i.e.,  converting the integer $n$ to the string $x$)
are ones of the core operations that require stronger solver support.
...
In fact, for many languages such as Java, the string-number conversion operations are is a niche operation, and much symbolic execution can not  be done without supporting it. For example, the program fragment below is a variant of the Luhn test algorithm \cite{?} that is often used in credit card or personal ID validation.
...
Currently, even for a simple constraint like the above one, most solvers already fail to provide a correct answer. Therefore, there is an urgent need for developing efficient techniques for solving the combination of above constraints. To make matters worse, many scripting languages, of which JavaScript is the most notable ---it powers most interactive content on the Web, and increasingly server-side code with Node.js---, has conversions between strings and integers embedded in the core of its semantics. To see how, consider the following program:
...
A casual glance at the above code reveal no use of strings at all, but the semantics of field access is somewhat unusual in JavaScript: the arrays are indexed by strings, and numeric indexes are converted to strings. This conversion is mandated explicitly by JavaScript semantics: the 2019 edition of ECMAScript \ref{?}requires that $ToPropertyKey$ be called on the element expression (\S{12.3.2.1}), and $ToPropertyKey$ calls {\tt{ToString}} on that value in all but special cases (\S{7.1.14}). Any faithful symbolic execution of JavaScript must handle such conversions for even basic array operations to work correctly; consider the following code snippet that manipulates an array \texttt{x}, with its value shown on the right:
...
Solving string constraint with string-number conversion is a very challenging problem. From the theoretical point of view, this problem is already proven to be undecidable~\cite{day2018satisfiability}. In practice, in our experiments (Section \ref{section:evaluation}), the state-of-the-art string constraint solvers provide only limited support to string-number conversion. 

In this paper, we propose a framework that handles string constraints with string-number conversion efficiently. Due to the high complexity of solving the exact problem, our framework uses both over and under-approximation of the string constraints. The over-approximation is for proving UNSAT and the under-approximation is for proving SAT of the string constraint. Both over and under-approximation  fall in a decidable fragment of string constraints that can we can efficiently solved. 
...
For proving UNSAT, our framework does not set any restriction on which decision procedure to use. One can select any decision procedure they want to use and ``over-approximate'' the input constraint to one that falls in the decidable fragment, e.g., the acyclic fragment~\cite{abdulla2014string}, the straight-line fragment~\cite{chen2019decision}, or the chain-free fragment~\cite{abdulla2019chain}. For instance, we can convert $xy=yx$ to two formulas $\{x_1=xy, x_2=yx\}$ using two auxiliary variables $x_1,x_2$ and replace the constraint $n=\sti{x}$ with $n=-1  \Leftrightarrow x\not\in [0-9]^*$, where $\Leftrightarrow$ means ``if and only if''. This produce new formulas that over-approximate the original ones and fall in the chain-free fragment~\cite{abdulla2019chain}, which is decidable. Since $\Phi$ is satisfiable, it is expected that the over-approximation module will not return UNSAT and we proceed to the the under-approximation module to prove SAT.

In our implementation \tool, we further add heuristics trying to derive the constant value of any side of the constraint $n=\sti{x}$ to refine the over-approximation. For instance, assume in some other cases, we can derive $n=12$ from some integer constraints. In such case, we can further derive that the value of $x$ is in the regular language $(0^*12)$. 

For proving SAT, our procedure first restrict the domain of each string variables to the language defined by some parametric flat automata (PFA) and then convert the string constraints under the domain restriction to equisatisfiable linear formulas.  Here we try to explain what the linear formula looks like by an example. Assume that we project the domain of $x$ and $y$ to the PFA in Figure~\ref{fig:sfa} (a) and (b), respectively. The variables $v_1$, $v_2$, $v_3$, $v_4$ in the figure are \emph{character} variables. One special treatment we do in this paper is that we encoding characters as natural numbers. So $v_1$, $v_2$, $v_3$, $v_4$ are also integer variables. For example, from the formula $ y \in (12)^+$, we may derive $v_3=1$ and $v_4=2$. 
...
\yfc{maybe add a flowchart later if space allows}
...
%\yfc{Is $\parikhof{A}$ defined only over $\#I$? Does this work for Corollary~\ref{corollary:pfa}?}

\begin{lemma}\label{lemma:decx}
%For a PFA $A$, $L(A) = \{\decode A(\pim) \mid \pim\in \parikhof A\}$.
...
The following corrolary of Lemma~\ref{lemma:decx} states that encodings of words in $\semof{A}$ are encodings of parametric words of $L(A)$ paired with arbitrary interpretations of character variables $\cvar$ that satisfy $\pfaf$:%, which are precisely all models of the formula $\parikhfof A$ interpreted over $\cvar\cup\pvarsof \cvar$, as formulated by the following corollary of Lemma~\ref{lemma:decx}:
%
...
%$\semof{A} = \{(\decode A(I\cup\pim)) \mid \pim\in\parikhof A,I:\cvars\rightarrow\Sigma_\epsilon\}$.
\\
...
%$\encode A (L) = \{(I\cup\pim) \mid \decode A (I\cup \pim) \in L \cap \semof{A}\}$. 
%We have that $\decode A \circ \encode A$ equal the identity function.
%
...
% and $\underf R {\neg\phi} \defeq \neg\underf R{\phi}$. The underapproximation formula is then in the form $\outlf \defeq \underf R {\insc}$.
...
%pay attention to how $\underf R \phi$ atually encodes the models of $\phi$, and
...
%\semof {\syncfof A {A'}}_{\cvarone\cup \cvartwo \cup \pvarone\cup\pvartwo} =
%%\semof {\syncfof A {A'}} \iequiv
%\{(I\cup I_\#)\cup (I' \cup I'_\#)\mid (I\cup I_\#) \in \encode A (w), (I' \cup I'_\#)\in\encode {A'} (w),w\in\semof A \cap \semof {A'}\} 
%$
$
\semof {\syncfof A {A'}}_{\cvarone\cup \cvartwo \cup \pvarone\cup\pvartwo} =
...
$
\end{lemma}
%\lh{the lemma is actually true even for two non-flat automata. Notice that in that case, the set of encodings does not correspond to the intersection precisely}
...
we use restric the semantics of $\syncfof A {A'}$ to the original variables $\cvarone \cup \cvartwo \cup \pvarsof{\cvarone} \cup \pvarsof\cvartwo$. %This is  achieve by quantifying out the variables of $V_\times$.
...
%$\semof A \cap \semof {A'} = \{\decode A(I\cup \pim) \mid (I\cup \pim) \in \semof{\syncfof A {A'}}_{\cvarone\cup \pvarone}\}$.  
...
%\{(I\cup I_\#)\cup (I' \cup I'_\#)\mid$ $\\(I\cup I_\#) \in \encode A (w), (I' \cup I'_\#)\in\encode {A'} (w'),w\in\semof A,w'\in \semof {A'}\}$
...
%\end{multline*}
\end{lemma}
\lh{lemma ugly, unnecesarily complicated. Just remove this lemma?}
...
%The resulting under-approximation of $\Psi_r$ is then the formula $\syncfof{A_x} {A'} \land \Psi_{char}$, which will be added to $\Phi_O$.
Then 
$$\underf R {\phi_r} = \syncfof{A_x} {A'} \land \Psi_{\mathit{char}}$$ 
%, which will be added to $\Phi_O$.
...
%Intuitively, it says that the resulting constraint represents precisely the restriction of $L(A)$ to the domain $\semof{A_x}$, the $A_x$-encodings of the strings in $L(A)\cap\semof{A_x}$. 
\begin{lemma}\label{lemma:memcorrect}  $\semof {\underf R {\phi_r}}_{V_R \cup V_{\#R}} = \encode {R} (\semof{\phi_r})$ . 
\end{lemma}

%
%\begin{lemma}\label{lemma:memcorrect}  ${\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}} = \{(I\cup\pim) \mid \decode {A_x}(I\cup\pim) \in L(A)\}$. 
%\end{lemma}
%
%(An alternative description)
%\begin{lemma}\label{lemma:memcorrect} For all $w\in \semof{A_x}$, %$(I\cup\pim)$ in ${\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}}$ such that $w =\decode {A_x}(I\cup\pim)$ if and only if $w \in L(A)$.
%%	$$\exists (I\cup\pim) \in {\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}}. w =\decode {A_x}(I\cup\pim)$$
%	\begin{center}
%	$\exists (I\cup\pim)$ in ${\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}}$ s.t. $w =\decode {A_x}(I\cup\pim)$\\
%		iff\\
%		$w \in L(A)$.
%	\end{center}
%%	${\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}} = \{(I\cup\pim) \mid \decode {A_x}(I\cup\pim) \in L(A)\}$. 
%\end{lemma}
%
%\lh{
%\begin{lemma}
%${\semof {\syncfof {A_x }{A'} \land \Psi_{char}} } _{V_x \cup \pvarsof{V_x}} = \encode {A_x}(L(A))$
%\end{lemma}
%}
...
%\subsection{Handling Equality Constraints Symbolically, Experimental version} \label{section:eq}
...
Last, we decribe the construction of $\underf R {\phi_s}$ for a string-number conversion constraint $\phi_s \defeq n=\sti{x}$. %In this section, we how to make the corresponding linear formulas for $\Phi_O$.
...
$$ 
and $\Psi_{\mathsf{Number}}$ caputures the case when $x$ is a valied encoding of a number. $\Psi_{\mathsf{Number}}$ aslo forces $R(x)$ to encode the number in a particular way, namely, the intial self-loop may only generate trailing zeros and %variables that are assigned $\epsilon$ can only be used at the end of the number: 
the first $k$ variables are some numbers and those after $k$ are assigned $\epsilon$. 
%Recall that we use $-1$ to represent $\epsilon$. The function $\mathsf{toNumber(k)}$ converts the first $k$ variables to the corresponding number in $n$. \yfc{say shifting $\epsilon$ to the right will not change the language}
