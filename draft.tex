...
we will formulate the corresponding lemma for linear formulas constructed from each type of atomic string constraints.
%
...
A minor technical difficulty is that we wish to build under-approximation in the form of pure arithmetic formulas, 
...
Notice that if $\pa$ is flat (or, symmetrically, if $\pa'$ is flat), then the semantic intersection $\semof \pa \cap \semof {\pa'}$ can be still decoded from the synchronization formula $\semof {\syncfof \pa {\pa'}}$. 
...
the $\epsilon$-transition is labeled by a fresh variable $v_\epsilon$ restricted by the constraint $v_\epsilon = \epsilon$.  
...
Length constraints are not under-approximated by flattening. 
Given a length constraint $\phi_l$ with string length variables $X$,
...
$$
The lenght of the encoded word $x$ is then the sum of the lengths contributed by the individual character variables in $V_x$, 
hence we let 
$$
...
It is not difficult to see from the examples above that, if $R(x)$ is an arbitrary PFA with $m$ loops, the formula to defines the number $n$ contains at least $m$ exponential components, one for each loop.
