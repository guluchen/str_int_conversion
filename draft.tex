...
The \emph{Parikh image} of a string $w\in \Sigma^*$ is the assignment 
...
Their implication central for our method is that every parametric string $x\in L(A)$ is uniquely determined by its Parikh image $\parikhof{x}$, as formalized by the following lemma.  
...
\yfc{Maybe we use things like $I_{\#}$ to replace $\pim$? It reads a bit strange when you put together $I$ and $\pim$, one is greek symbol and the other is roman, but both of them are interpretations.}

Since every string $w \in \semof{A}$ is obtained from a parametric string $x\in L(A)$ and an interpretation of character variables $I:\cvar\rightarrow\Sigma_\epsilon$ such that $w = I(x)$, and since $x = \decode A(\parikhof x)$, 
...
The following construction is an important step in building under-approximations of string constraints. 
For two PFA $A$ and $A'$ over character variables $V$ and $V'$, respectively, 
it builds \emph{synchronization formula} $\syncfof A {A'}$, a linear formula over $\cvarone\cup \cvartwo \cup \pvarone\cup\pvartwo$ encoding the semantic intersection $\semof A \cap \semof {A'}$. 
...
The correctness of this construction is expressed by Lemma~\ref{lemma:synccorrect} below. The lemma can also be understood as a precise specification the semantics of the synchronization formula, on which we rely for correctness of the construction of underapproximations of string constraints. 
%
