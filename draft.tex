...
we may say that the united assignment $I \cup \parikhof x$ encodes $w$. Lemma~\ref{lemma:parikh} also implies that two different strings are encoded by different united assignments, and hence we may extend the decoding function $\decode$ to united assignments so that $\decode(I \cup x) = w$.
Encodings of strings in $\semof{A}$ are encodings of strings of $L(A)$ paired with arbitrary interpretations of $\cvar$, 
hence $\semof A$ is characterised by $\parikhof A$ interpreted over variables $\cvar\cup\pvars$: 
...
Synchronisation formula is a formula $\syncof A B$ over $\pvars\cup\cvarone\cup \cvartwo$ encoding the semantic intersection $\semof A \cap \semof B$. Besides the intersection, it contains a more precise information about the synchronisation of $A$ and $B$, 
...
Assume that $A$ and $B$ are over disjoint sets of variables. 
We build so called asynchronous product machine of $A$ and $B$ and then extract the formula from its Parikh image.
%
The product machine uses $Q\times P$ as the set of states and $(\cvarone \cup \{\epsilon\}) \times (\cvartwo \cup \{\epsilon\} )$ as the alphabet. If a transition $((q_1,p_1), (v,v'),(q_2,p_2))$ is taken, it means the character variable $v$ and $v'$ should be assigned the same value and this value leads $S_n$ from state $q_1$ to state $q_2$ and $S_m$ from $p_1$ to $p_2$.

More precisely, the product machine is a tuple $M = (Q\times P, T^2, (\cvarone \cup \{\epsilon\}) \times (\cvartwo \cup \{\epsilon\} ), (q_i,p_i),(q_f,p_f))$, where the transition relation $T^2$ is the minimal set satisfying the following.

\begin{itemize}
\item If $(q_1,v,q_2) \in T$, then for all states $p\in P$, we have $((q_1,p),(v,\epsilon),(q_2,p))\in T^2$.
\item If $(p_1,v,p_2) \in T'$, then for all states $q\in Q,((q,p_1)$, we have $(\epsilon,v),(q,p_2))\in T^2$.
\item If $(q_1,v,q_2) \in T$ and $(p_1,v',p_2) \in T'$, then we have $((q_1,p_1),(v,v'),(q_2,p_2))\in T^2$.
\end{itemize}	
...
%
We compute a Presburger formula $\pformulaof M$ over the set of variables $\{\#(v,v')\mid (v,v') \in \cvarone \times \cvartwo\}$ of the product machine. 
%We use $\#v$ and $\#v'$ to denote the number of occurrences of $v_i$ and $v_i^{(j)}$, respectively. 
The following formulae establish the relation of the number of occurrences of symbols between the SFAs and the product machine.
\begin{enumerate}
...
we have $(I\cup\pim)\cup(I'\cup\pim')\in\semof \Psi$ if and only if $\decode(I\cup\pim) = \decode(I'\cup\pim')$.
\end{lemma}
We actually have even something stronger, namely, that, $\Psi$ has all possible allignments. How do I say this ... or do I have to say this?
