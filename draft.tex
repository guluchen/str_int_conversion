...
namely in the form of\\ $\semof A \cap \semof {A'} = \{\decode A(I\cup \pim) \mid \exists J.((I\cup \pim) \cup J) \in \semof{\syncfof A {A'}}\}$.  
\bigskip
...
Assume that the constraint we take from $\Phi_I$ is a regular constraint $\phi_r \defeq x \in L(A)$. Assume that $A=(Q,T,\Sigma,q_i,q_f)$. First we create a PFA $A' = (Q,T', V',q_i,q_f)$ that has exactly the same set of states as $A$. The set of transitions $T'$ and character variables $V'$ can be obtained by the following procedure.
Initially, both $V'$ and $T'$ are empty sets.
For each transition $(q, a, q') \in T$, we add a fresh character variable $v$ to $V'$, add the transition $(q,v,q')$ to $T'$, and add the constraint $v = \modelsof{a}$ to $\Phi_O$. Recall that $\modelsof{a}$ is the numerical encoding of the character $a$. After $A'$ is built, we add the synchronization formula $\syncfof {A_x }{A'}$ to $\Phi_O$. 
...
\begin{lemma}\label{lemma:memcorrect}
	%$\semof {\exists V_\times . \syncfof A {A'}}\restrict{\cvarone \cup \cvartwo \cup \pvarsof{\cvarone} \cup \pvarsof\cvartwo} = \{(I\cup\pim)\cup(I'\cup\pim') \mid \decode A(I\cup\pim) = \decode {A'}(I'\cup\pim')\}$. 
	${\semof {\syncfof {A_x }{A'}}}_{V_x \cup \#V_x} = \{(I\cup\pim) \mid \decode {A_x}(I\cup\pim) \in L(A)\}$. 
\end{lemma}
