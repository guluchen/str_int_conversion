...
a \emph{flat domain restriction} for $\insc$ is a mapping $R$ that assigns to every variable $x\in X$ a PFA $R(x)$ \changed{over character variables $V_x$.}
string %
We require that these PFA operate over pairwise disjoint sets of character variables\changed{, that is if $x\neq y$ then $V_x \cap V_y = \emptyset$.}
%
...
%
\changed{
Further, we define an $R$-encoding for an interpretation $I \cup I'$ of a general string constraint, consiting of an interpretation $I$ of string variables and an interpretation $I'$ of integer variables.
%
An $R$-encoding of $I \cup I'$ is a union  $e \cup I'$ of an encoding where $e\in \encode R(I)$ and the original integer interpretation $I'$ which satisfies that for every string leght variable $|x|$ in the domain of $I'$,
$|x| = \sum_{v\in V_x,I(v)\neq\epsilon}\#v$. 
That is, the interpretation of string variables is encoded as usual, the interpretation of integer variables is kept as is, 
and for each string legth variable $|x|$ in the domain of $I'$, 
the value of $|x|$ is consistent with the encoding of $x$ by variables $V_x$ and $\#V_x$.
The set of all $R$-encodings of $(I\cup I')$ is then denoted $\encode R {(I\cup I')}$}
%
...
%
%
and we lift this notation to sets of interpretation in the standard manner, i.e., $\encode R (J) = \bigcup_{I \in J}\encode R (I)$ for a set of interpretations $J$.
%
