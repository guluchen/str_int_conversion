...
In this paper, we report a novel approach for solving string constraints with string-number conversion and implemented it as an open-source tool {\tool}. For now, it support basic string constraints, string-number conversion, and also operations that can be encoded to them (e.g., \textsc{contains}, \textsc{prefixOf}). Since {\tool} is built inside the SMT solver Z3, we also get the power of processing formulae in the combination of different theories (e.g., array) and hence can support the encoding of a wide range of program expressions. There are several avenues for future works. We are planning to integrate it with the JavaScript symbolic executor \textsc{cosette}, such as the one in~\cite{santos2018symbolic}. We have discussed this possibility of such integration with some of its team members, and we believe it is feasible. We are also planning to merge it to the main branch of the Z3 solver, to increase the visibility of {\tool}. For technical development, we think it would be interesting to consider the flattening of an even larger set of string operations, such as \textsc{replaceAll} and \textsc{split}. 
