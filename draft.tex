...
We pick a PFA for each variable in $\vars$ at the beginning of the decision procedure and then construct the set of linear integer constraints in a stepwise manner. In each step, we take a string constraint from $\Phi_I$ and depending on the type of the string constraint, we execute the corresponding procedure to produce the linear integer constraints and add them to the output linear formulas $\Phi_O$. We also add constraints to $\Phi_O$ to ensure all character variables of the PFAs only pick values from $\Sigma_\epsilon$.
...
every parametric word $x\in L(A)$ is uniquely determined by its Parikh image $\parikhof{x}$, i.e., $\decode A (\parikhof x)) = x$, and analogously for the entire language of $A$, as formalized by the following lemma.  
...
We may say that $\parikhof x$ is an encoding of $x$, and define the decoding function $\decode A$ such that $\decode A(\parikhof x) = x$, and we lift the function to sets of integer assignments in the standard manner.
%
...
For a PFA $A$, $L(A) = \decode A(\parikhof{L(A)}$.
\end{lemma}
%\lh{should we replace $\semof{\parikhfof A}$ by $\parikhof{A}$ here and almost everywhere?}
%\yfc{Is $\parikhof{A}$ defined only over $\#I$? Does this work for Corollary~\ref{corollary:pfa}?}
%
...
$\semof A = \decode A \{(I\cup \pim)\mid \pim\in\parikhof{L(A)} ,I:\cvars\rightarrow\Sigma_\epsilon\}$
\end{corollary}
%
...
\subsection{Flat Domain Restriction}
Let us now give a precise definition of how we restrict the domain of string constraints using flat automata. 
A \emph{flat domain restriction} for a string contraint $\phi$ over variables $X$ is a mapping $R$ that assigns to every variable $x\in X$ a PFA $R(x)$. 
For technical reasons that will be apparent further in this section, we require that if $x\neq  y$, then $R(x)$ and $R(y)$ have disjoint sets of character variables. We will use $V_R$ to denote the union of character variables of all $R(x),x\in X$.

The semantics of a string contraint $\Phi_I$ over a set of variables $X$ restricted by $R$ is defined as 
$\semof\phi^R = \{I\in\semof \phi \mid \forall x\in X:I(x)\in\semof{R(x)}\}$. 
In the rest of this section, we will show how to build an \emph{underapproximating formula} representing the semantics $\semof{\Psi_I}^R$ of an input string constraint, that is, a formula $\Psi_O$ such that 
$\{\decode R (\semof {\Psi_O})\} = \semof{\Psi_I}_R$. 

The formula $\Psi_O$ is build intuctively to the structure of $\Psi_O$.
Namely, in the followin subsections we show how to build a formula $\underf R {\phi}$ for every atomic string constraint $\phi$, and for a Boolean combination of 
string constraints we have $\underf R {\phi \land \phi'} = \underf R {\phi} \land \underf R {\phi'}$ and $\underf R {\neg\phi} = \neg\underf R{\phi}$. The underapproximation formula is then in the form $\Psi_O \defeq \underf R {\Psi_I}$.

The correctness of this construction, that is, that 
$$\decode R (\semof {\Psi_O}) = \semof{\Psi_I}^R$$ can be proved by induction to the structure of $\Psi_I$. However, for the induction step to go through, we will need a more meticulous view into the correspondence of $\phi$ and $\underf R \phi$. Particularly, we will need to
...
Given a PFA $A$, and a language $L\subseteq \Sigma^*$, we define the set of $A$ encodings of $L\cap \semof A$ as $\encode A (L) = \{(I\cup\pim) \mid \decode A (I\cup \pim) \in L \cap \semof{A}\}$.
%
For an interpretation $I:X\rightarrow \Sigma^*$, we define its $R$-encoding as the set $\encode R(I) = \bigcup_{x\in X} \encode A(I(x))$. We left this notation to sets of interpretation in the standard manner, i.e., $\encode R J = \bigcup_{I \in J}\encode R (I)$ for a set of interpretation $J$.
%
The needed inductive argument of the correctness of the construction can be expressed as that 
$$\semof{\underf R \phi}_{V_R\cup V_{\#R}}  = \encode R{\semof{\phi}}$$ 
We restrict need to restrict here interpretations of $\underf R \phi$ since its construction uses some additional variables. 

%\subsection{Handling Regular Constraints Symbolically} \label{section:mem}
\subsection{Underapproximation of Regular Constraints} \label{section:mem}
...
We now describe the construction of $\underf R {\phi_e}$ for an equality constraint $\phi_e\defeq x_0\cdot x_1 \cdots x_n = x_{n+1}\cdot x_{n+2} \cdots x_m$. We first build two PFAs $\leftA$ and $\rightA$ that encode the left-hand side term and the right-hand side term of the equality constraint. Intuitively, this can be achieved by connecting the PFAs restricting the string variables with epsilon transitions. However, there are two technical issues. First, our definition of PFA does not allow epsilon transition. %The second issue is more severe, 
Second,
...
$$
\underf R {\phi_2} \defeq \Psi_\epsilon \land \Psi_{\mathit{rename}} \land \syncfof \leftA \rightA
$$
...
Remember that the character variables can be assigned $\epsilon$. In order to simplify the construction, we use an encoding to shift all the $\epsilon$-transitions to the least significant digits, i.e., if $v_j = \epsilon = -1$ then $v_k = -1$ for all $k\in [j+1,m]$.  To explain this encoding, we first define two macro function:
...
$$
\underf R {\psi_s} \defeq \Psi_n \wedge \Psi_{v_0}\wedge \parikhfof {A_x} 
$$
...
After the set $\Phi_I$ becomes empty, we start to add constraints to $\Phi_O$ to relate the lengths of string variables and their PFA. To do so, we first define $\Phi_{V_x}$ that describes the number of occurrences of each non-epsilon character variable in $x$.

$$\Phi_{V_x} \defeq \bigwedge_{v\in V_x} ( (s_v = 0 \wedge v = -1) \vee (s_v = \#v \wedge v \neq -1)) $$


More concretely, for each string variable $x$, we add linear formula $\Psi_{|x|}$ to $\Phi_O$. 
