...
\end{itemize}
We use the linear formula $\Psi_c \defeq \bigwedge\limits_{(q,a,q')\in T} (v_{(q,a,q')} = a)$ to remember the relation between the fresh character variables in $V'$ and the symbol it represents. \lh{why  the name $\Psi_c$?} We add both $\syncfof {A_x }{A'} $ and $\Psi_c$ to $\Phi_O$. The relation between $A$ and $A'$ is expressed in Lemma~\ref{lemma:fa_pfa}.
...
%>>>>>>> 67b962e5df2a0689e472f2525f44d512d2c23dbd
%
...
\lh{Sketch of a simplified version:
...
We represent the FA $A$ by a pair of a PA $A'$ and linear formula $\Psi_c$. 
...
The formula $\Psi_c$ then binds the fresh variables to the characters they represent, 
namely, $\Psi_c = \bigwedge_{a\in\Sigma_\epsilon} v_a = a$.
The resultinng overapproximation of $\Psi_r$ is then the formula $\syncfof{A_x} {A'} \land \Psi_c$.
...
Intuitively, it says that the resulting constraint represents presicely the restriction of $L(A)$ to the domain $\semof{A_x}$, the $A_x$-encodings of the strings in $L(A)\cap\semof{A_x}$. 
%

%
\begin{lemma}\label{lemma:memcorrect}
	${\semof {\syncfof {A_x }{A'} \land \Psi_c} } _{V_x \cup \pvarsof{V_x}} = \{(I\cup\pim) \mid \decode {A_x}(I\cup\pim) \in L(A)\}$. 
\end{lemma}
}
