...
\changed{
We say that a mapping $e:\cvar\cup\#\cvar \rightarrow \nat$ is a \emph{word encoding} a word $w$ (or a $\pa$-encoding of $w$) if $e = \parikhof{x}\cup I$
...
\changed{
It uses the decoding function $\decode \dr$ parameterized by the domain restriction $\dr$. The $\decode \dr$ function maps a word encoding $I_e: V_\dr\cup\#V_\dr\rightarrow \nat$ paired with an interpretation of integer variables $I_z$ to a model $I_s \cup I_z$ of a string constraint. Formally, $\decode  \dr (I_e\cup I_z) = I_s \cup I_z$, where $x\in X$,
$I_s(x) = \decode {\dr(x)} ((I_e)_{V_{\dr(x)}\cup \#V_{\dr(x)}})$. 
	
}

\yfc{It uses the decoding function $\decode \dr$ parameterized by the domain restriction $\dr$. 
