...
%
It is extracted from the asynchronous product of $A$ and $A'$, an automaton $\syncof A {A'}$. 
%
The asynchronour product is an automaton that uses $Q\times Q'$ as the set of states and 
...
The synchronisation formula $\syncof A {A'}$ is extracted from $\syncof A {A'}$ as follows.
Its first, core part is the Parikh formula $\parikhfof {\syncof A {A'}}$ of the product, which encodes all runs of the product.
%
...
$$
The correctness of this construction is expressed by Lemma~\ref{lemma:synccorrect}. The Lemma can also be understood as a precise specification the semantics of the synchronisation formula. 
%
