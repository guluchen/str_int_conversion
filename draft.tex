...
to explain the main ideas behind our decision procedure.  To make our terminology explicit: $\Phi$ states that $x$ concatenated with $y$ is the same as $y$ with $x$, $n$ is the numeric value of the string $x$, $n > 3$, $y$ is a string of 1's and 2's.  Notice that $\Phi$ is satisfiable. E.g., it has a model $x=``12''$ and $y=``1212''$.


Our decision procedure has two steps: the first step is over-approximating the set of input constraints into a set that falls in the chain-free fragment~\cite{abdulla2019chain}, which is decidable. Observe that we could  over-approximate the input constraint into any decidable fragment, e.g. the acyclic fragment~\cite{abdulla2014string} or the straight-line fragment~\cite{chen2019decision}. Our choice of the  chain-free fragment~\cite{abdulla2019chain} is motivated by the fact that the chain-free fragment is the {\em largest} known decidable fragment for that class of string constraints. In our example, we over-approximate the formula $\Phi$ by converting $xy=yx$ to two formulae $\{x_1=xy, x_2=yx\}$ using two auxiliary variables $x_1,x_2$ and replace the constraint $n=\sti{x}$ with $n=-1 \vee (n\neq -1 \wedge x\in [0-9]^*$). The over-approximation module uses also some heuristics to derive the constant value of any side of the constraint $n=\sti{x}$ to refine the over-approximation. For instance, assume in some case, we could derive the value of $n=12$ from some integer constraints.  Then we could further derive the value of $x$ is in the regular language $(0^*12)$. 
...
To demonstrate the usefulness of our approach, we have implemented our decision procedure in an open source solver, called {\sc } ....
...
In fact, the the parikh image of a word $w\in L(A)$ can be seen as an encoding of $w$ and can be uniquelly decoded:
%i.e., $\decode A (\parikhof w)) = w$, 
%and analogously for the entire language of $A$, as formalized by the following lemma.  
...
\emph{Parametric flat automaton (PFA)} then is a parametric automaton with its finite automaton flat.
...
%
a mapping $I:\cvar\rightarrow$ to $\Sigma_\epsilon$. 
For a parametric word $x= v_1v_2\cdots v_k$ over $\cvar$, its interpretaion $I(x)$ is then defined as $I(v_1)\cdot I(v_2)\cdot \ldots \cdot I(v_k)$.
%
...
Since every word $w \in \semof{\pa}$ is uniquelly determined by a parametric word $x\in L(A)$ and an interpretation $I\in\semof{\paf}:\cvar\rightarrow\Sigma_\epsilon$ such that $w = I(x)$, and since $x$ is uniquelly determined by its Parikh image, i.e.,  
$\decode \pa(\parikhof x)$, 
we may say that the assignment $I \cup \parikhof x$ uniting the interpretation $I:\cvar\rightarrow\nat$ and the Parikh image $\parikhof x:\#\cvar\rightarrow \nat$ \emph{$\pa$-encodes} $w$. Lemma~\ref{lemma:decoding} implies such $\pa$-encoding can be uniquelly decoded. 
...
We again lift the decoding function into sets of united assighnments in the standard manner.
The following corrolary of Lemma~\ref{lemma:decoding} states that encodings of words in $\semof{\pa}$ are encodings of parametric words of $L(A)$ paired with arbitrary interpretations of character variables $\cvar$ that satisfy $\paf$:
...
Last, we define the encoding function as a counterpart of the decoding. Given a PFA $\pa$, and a language $L\subseteq \Sigma^*$, we define the set of $\pa$-encodings of the words in $L$ as 
$\encode \pa (L) = \{e \mid \decode \pa (e) \in L\}$. 
%
Notice that only the words from $L\cap \semof \pa$ have an $\pa$-encoding, 
...
and start the description of the algorithm that converts a string constraint $\insc$ to a linear formula $\outlf$ representing the restricted set of solutions.  
%

The domain restriction is formally defined by restricting the domain of each string variable to a chosen flat language. 
%
...
for each variable  depends on the stratedy used in the implementation, and will be discussed in Section~\ref{section:implementation}.
%
The formula $\outlf$ will be  built intuctively to the structure of $\insc$.
...
The following sections will be devoded to describing the construction of the formula $\outlf$ representing the $R$-restriction of $\insc$, that is, such that the following theorem holds: 
The correctness of this construction is expressed by  Theorem~\ref{theorem:correct}:
...
For an interpretation $I:X\rightarrow \Sigma^*$, we define its $R$-encoding as the set $\encode R(I) = \bigcup_{x\in X} \encode \pa(I(x))$. We lift this notation to sets of interpretation in the standard manner, i.e., $\encode R (J) = \bigcup_{I \in J}\encode R (I)$ for a set of interpretations $J$.
%
...
%
In the nexr sections, 
we will formulate the corresponding lemma for linear formulae constructed from each type of atomic string constraints.
%
We note that the restriction of the semantics of $\underf R \phi$ here is needed since $\underf R \phi$ is constructed with some auxiliary variables. 
...
We will first discuss flattening of the basic string contraints, that is,
regular, equality, and lenght constraints. We start by two needed operations over PA, synchronisation and concatenation.
...
The correctness of the construction of under-approximations of equality constraints and regular constarints in Sections~\ref{section:mem}, \ref{section:eq} rely on it. 
%
...
Since that the models of $\syncfof \pa {\pa'}$ include also an assignment to the auxiliary variables of $(\cvarone_\epsilon\times\cvartwo_\epsilon) \cup \pvarsof{(\cvarone_\epsilon\times\cvartwo_\epsilon)}$, the statement of the lemma, 
restrics the semantics of $\syncfof \pa {\pa'}$ to the original variables $\cvarone \cup \cvartwo \cup \pvarsof{\cvarone} \cup \pvarsof\cvartwo$. %This is  achieve by quantifying out the variables of $V_\times$.

Notice that if $\pa$ is flat (or, symmetrically, if $\pa'$ is flat), then the semantic intersection $\semof \pa \cap \semof {\pa'}$ can be still decoded from the synchronisation formula $\semof {\syncfof \pa {\pa'}}$. 
...
With syncrhonisation and concatenation of PA, we are ready to describe flattening of the basic string constraints.
...
In order to synchronise the FA $A$ with the PFA $R(x)$, 
...
%
To simplify the presentation, we assume that the varriables are pairwise different, i.e. that $i\neq j \implies x_i \neq x_j$. We may make this assumption without loss of generality, since multiple occurences of variables in $\phi_e$ can be eliminated: whenever $x_i = x_j$ for $i \neq j$, we may replace $x_j$ by a fresh variable $x_j'$ and conjoin the modified $\phi_e$ with a new equality $x_j = x_j'$. 
%
We also that all disequality constraint $t \neq t'$ are already converted to equal-satisfiable equality constraints and integer constraints in the standard way~\cite{abdulla2015norn}.
%
...
\subsection{Encoding Length Constraints}
Length constraints are not underapproximated by flattenting. 
Given a length constraint $\phi_l$ with string lenght variables $X$,
we keep $\phi_l$ as is and add a formula to express the lenghts of strings $|x|$ in terms of character variables $V_x$ and Parikh variables $\# V_x$ of $R(x)$.

For every length variable $|x|\in X$, 
we add a set of fresh auxiliary variables $\{|v|\mid v\in V_x\}$ to express the length by which $v$ contributes to the lenght of an $R$-encoded word. 
...
$$
The lenght of the encoded word $x$ is then the sum of the lenghts contributed by the individual character variables in $V_x$, 
hence we let 
$$
...
$$
\lh{a correctness lemma would be good here, but it needs some modification of encoding probably, to be done}
...
Last, we present a central part of this paper, the construction of $\underf R {\phi_s}$ for a string-number conversion constraint $\phi_s \defeq n=\sti{x}$. 
...
It is not difficult to see from the examples above that, if $R(x)$ is an arbitrary PFA with $m$ loops, the formula to defines the number $n$ contains $m$ exponential components, one for each loop.
...
However, the formula is satisfiable when $x=``00010"$. Observe that $\sti{``00010"}=10$. The key is that even for a bounded integer, the corresponding string can be of unbounded length with arbitrarily many trailing `$0$'s at the front. All numbers with up to $m$-digits can be however still handled without having to solve exponential constraints. It is enough to equip the initial state of the PFA with a  $0$-self-loop.
...
$$ 
to hold the value $0$. 
This transition ensures that the under-approximation handles all numerals with arbitrary number of trailing zeros.  
The self-loop is followed by a chain of $m$ transitions $(q_{i-1},v_i,q_{i}),1\leq i \leq m$, leading towards the final state $q_m$. The chain encodes at most $m$ meaningful digits (only \emph{at most} because the first variables in the sequence can still be assigned zeros and some variables may be assigned $\epsilon$). Hence this PFA covers all numerals that encode numbers with at most $m$ digits.
...
%
\lh{I am not sure whether it is really impossible to make a smlall formula}
%
This would lead to an exponentail expolostion of the size of the linear formula definig the value of $n$.
%
...
$$
Last, since this restriction is meanigfull only when the string is indeed numeral,  
...
$
\end{lemma}
The lemma uses the encoding function generalised to assignments of string \emph{and integer} variables. 
It is defined so that, for an interpretation $I$ of string variables $X$ and an interpetation $I'$ of integer variables $N$, $X\cap N = \emptyset$, 
$\encode R' {(I\cup I')} \defeq \{e \cup I' \mid e\in \encode R(I)\}$, which is then lifted to sets in the standard manner. 
