...
\lh{or really have just an equivalence of semantics indexed by variables?}
...
\lh{What about to put this somewhere, it will simplify formulation of the correctness lemmas. Roughly, formulation need to be polished a and explained:}
Given a PFA $A$, and a language $L\subseteq \Sigma^*$, we define the set of $A$ encodings of $L\cap \semof A$ as
the set $\encode A (L) = \{(I\cup\pim) \mid \decode A (I\cup \pim) \in L \cap \semof{A}\}$.
%
A domain restriction $R$ for a string contraint $\phi$ over variables $X$ is a mapping that assigns to every variable $x\in X$ a PFA $R(x)$ such that for different variables $x,y \in X$, $R(x)$ and $R(y)$ have disjoint sets of variables. 
%
For an interpretation $I:X\rightarrow \Sigma^*$, we define its $R$-encoding as the set $\encode R(I) = \bigcup_{x\in X} \encode A(I(x))$, and we left this notation to sets of interpretation in the standard manner, i.e. $\encode R J = \bigcup_{I \in J}\encode R (I)$.
%
In the following section, we will discuss how, given a string contraint $\phi$ and a domain restriction $R$,
to build an arithmetic formula $\Psi$ such that $\semof\Psi_{V_R\cup\#V_R} = \encode R(\semof{\phi})$. 
Tha is, formula $\Psi$ will capture $R$-encodings of soltuions of $\phi$ under the domain restriction given by $R$.
...
We represent the FA $A$ by a pair of a PA $A'$ and linear formula $\Psi_{char}$. 
...
The formula $\Psi_{char}$ then binds the fresh character variables to the character values they represent, 
namely, $\Psi_{char} = \bigwedge_{a\in\Sigma_\epsilon} v_a = a$.
...
Assume that the constraint we take from $\Phi_I$ is an equality constraint $\phi_e\defeq x_1\cdot x_2 \cdots x_n = x_{n+1}\cdot x_{n+2} \cdots x_m$. We first build two PFAs that encode the left-hand side term and the right-hand side term of the equality constraint. Intuitively, this can be achieved by connecting the PFAs of the string variables with epsilon transitions. However, there are two technical issues. First, our definition of PFA does not allow epsilon transition. The second issue is more severe, some string variable might occur multiple times in an equality constraint, but the definition of PFA allows each character variable occurs in only one transition. We will need a way to force that all occurrences of the same string variable are assigned the same value.
...
For the second issue, we rename the variables on the transitions of PFAs for different occurrences of the same variable in one equality constraint. The renaming is done by adding a version number to the variable. We call variables before and after renaming the \emph{original variable} and the \emph{renamed variable}. For example, we can rename $v_1$ to $v_1^{(0)}$, $v_1^{(1)}$, etc. In this case, $v_1$ is the original variable and both $v_1^{(0)}$ and $v_1^{(1)}$ are the renamed variables. We will an add arithmetic constraint $\Psi_{rename}$ (will be defined later) to enforce that variables renamed from the same variable are assigned the same value. 
...
Now let $A$ and $A'$ over character variables $V$ and $V'$ be the two PFAs obtained from the two sides of the equality constraint $\phi_e$. We then construct the synchronization formula $\syncfof A {A'}$ and add it to $\Phi_O$. Recall that from Lemma~\ref{lemma:synccorrect}, each model of $\syncfof A {A'}$ corresponds to a pair of words in $\semof{A}$ and $\semof{A'}$. However, this does not directly correspond to a model of the equality constraint $\phi_e$. The reason is that different occurrences of the same string variables can be assigned different values in $\semof{A}$ and $\semof{A'}$. To avoid this problem, we need to add the constraint $\Psi_{rename}$ that is defined below to $\Phi_O$ to ensure that all variables renamed from the same original variable are assigned the same value and are occurred equally often.
...
Assume that the constraint we take from $\Phi_I$ is a string-number conversion constraint  $\phi_s \defeq n=\sti{x}$. In this section, we how to make the corresponding linear formulas for $\Phi_O$.
...
$$\Psi_{NaN} \defeq  \bigvee_{i\in [1,m]} v_i>9$$

Then we can specify the relation between $n$ and the character variables as follows:
$$\Psi_n\defeq  (n=-1 \wedge \Psi_{NaN})\vee \bigvee_{k\in [0,m]} (\mathsf{epsAfter(k)} \wedge \mathsf{toNumber(k)} )$$
