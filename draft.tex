...
\{
(I\cup\parikhof{w})\cup(I'\cup\parikhof{w'}) \mid 
w\in L(A), w'\in L(A'), I(w) = I(w')
\}
$
\end{lemma}
Notice that if $A$ is flat (or $A'$, symmetrically), then all the $w\in \semof A \cap \semof {A'}$ can be extracted from $\semof {\syncfof A {A'}}$ using the decoding function $\decodeof A$.  
\bigskip
