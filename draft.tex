...
The string-number conversion operations are widely used in string manipulating programs and algorithms. For example, the program fragment below is an implementation of the Luhn test algorithm that is often used in credit card or personal ID validation.
...
The input \verb|value| is a sequence of digits. The algorithm process the digits in the reversed order. The value of every odd digit (e.g., 1st, 3rd, etc.) is added to \verb|sum| directly. For the value of every even digit, the algorithm (1) doubles its value, (2) substracts its value by $9$ if the doubled-value is bigger than $9$, and (3) adds the final result to \verb|sum|. At the end, the input is validated if the string value of \verb|sum| ends in $0$.
...
Here both \texttt{x[3]} in line 2 and \texttt{x["3"]} in line 3 denote the array element of \texttt{x["3"]}, but \texttt{x["03"]} denotes a completely different element. So naive idea of modeling array index using integers will not work -- it cannot distinguish the indexes \texttt{"3"} and \texttt{"03"}. 

If we model array index with strings, then we need to face the second problem -- we need to handle arithmetics in the array index. Let us look at the case of line 4, we need to update the value of \texttt{x["03"-1]}. The expression \texttt{"03"-1} will be evaluated to $2$, which involves an implicit type conversion from the string \texttt{"03"} to an integer value $3$ due to the \texttt{-} operation. Then the number $2$ will be converted back to string \texttt{"2"} and used as array index and hence \texttt{x["03"-1]} is the array element of \texttt{x["2"]}. Even for a simple example like this, the conversion between string and number is unavoidable. This is a rather straightforward array operation, and so this limit will cripple any model of non-trivial JavaScript code. 
