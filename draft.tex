...
Concatenation is another operation over PFAs needed when constructing underapproximations. Its implementation is straightforward, connect the final state of the first PFA with the initial state of the second by an $\epsilon$-transition. Sinco our automata do not allow $\epsilon$-transition, 
the connecting transition is labeled by a fres variable $v_\epsilon$ and the constraint $v_\epsilon$ is added to the interpretation contraint.  
