...
\newcommand{\enc}[1]{[\![#1]\!]}
\newcommand{\parikh}{\mathit{Parikh}}
...
$\parikh(w)$ that maps every character $a\in\Sigma$ to the number of its occurences in $w$, i.e., $\parikh(a) = |w|_a$. 
The Parikh image of a language $L$ is the set of Parikh images $\parikh(L) = \{\parikh(w) \mid w \in  L\}$. 
...
Notice that only positive integer is supported in this semantics. This is the semantics used by most of the SMT solvers, and hence we follow it in this paper. The encoding has a benefit that it can also handle the case where $x$ is ``not a number'', using the condition $\sti{x} = -1$.
...
An \emph{interpretation over $\vars$ and $\varn$} is a mapping $I$ from $\vars\cup \varn$ to $\Sigma^* \cup \mathbb{N}$. We lift $I$ to terms (i.e., an element in $(\vars\cup \Sigma)^*$) in the standard manner. 
...
The purpose of the structural restrictions of SFA is to ensure that the words of $L(A)$ are uniquelly determined by their Parikh images, as formalised by the following lemma.  

\begin{lemma}
%Distict words from the language of an SFA must have distinct Parikh images.  
For an SFA $A$ and $x,y\in L(A)$ if $\parikh(x) = \parikh(y)$ then $x = y$. 
...
Since every variable appears on at most one transition, 
the number of repetitions of each cyclce equals the value each variable on the edges of the cycle has assigned by $\parikh(x)$ (notice also that $\parikh(x)$ must assign to all variables appearing within the same cycle the same value). 
