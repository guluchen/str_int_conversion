...
\lh{TODO: solve the where to add $-1$ problem}
...
$$
where $\semof{\parikhfof A}_{\cvar\cup\pvar}$ are models of $\parikhfof A$, assignments to $\pvar$, united with arbitrary interpretations of $\cvar$ \lh{this is ugly}\yfc{I totally agree}.

\paragraph{Synchronisation of PFA}
The following construction of the so called synchronisation formula of two PFA $A$ and $A'$ is a crucial component of building underapproximations of string constraints. 
...
We start with computing the Parikh formula $\parikhfof {\syncof A {A'}}$ of $\syncof A {A'}$ over the set of variables $\pvarsof{\cvarone_\epsilon\times\cvartwo_\epsilon}$ and then conjoin it with the following restricting constraints. 
...
\syncfof A {A'} = {(\cvarone\times\cvartwo)}.\,\parikhfof{\syncof A {A'}} \land \Psi_{\#} \land \Psi_=
$$
...
\lh{TODO: put this somewhere and explain}
